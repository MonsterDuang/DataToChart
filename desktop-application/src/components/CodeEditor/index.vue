<template>
	<codemirror ref="editor" :value="content" :options="codemirrorOptions" @input="handleInputChange" @blur="handleBlur" :class="fs" />
</template>
...
const handleBlur = () => {
	if (props.mode == 'css')
		content.value = beautify.css_beautify(content.value || '', {
			indent_size: 2,
		});
	else content.value = beautify(content.value || '', formatOptions);
